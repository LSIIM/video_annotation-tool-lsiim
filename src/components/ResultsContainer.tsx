...
interface Props {
  ...
  export default function ResultsContainer({
    annotation,
    mode,
    selectedOptions = {}, // Valor padrão vazio
    onSelectOption,
  }: Props) {
    ...
    useEffect(() => {
      if (mode === "edit") loadResults(); // Apenas em modo "edit"
    }, [resultOptions]);
  
    async function loadResults() {
      if (!onSelectOption) return; // Ignora se `onSelectOption` não for fornecido
          ...
          <>
            ...
            <div className="max-h-96 overflow-y-auto">
              {resultOptions.map((result, index) => (
                (mode === "edit" || selectedOptions[index]) && (
                  <div
                    key={index}
